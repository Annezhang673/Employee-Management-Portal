...
>("review/fetchBucket", async (status) => {
  const res = await axiosApi.get<AppRow[]>("/api/hiring/review", {
    params: { status },
  });
...
export const rejectApp = createAsyncThunk<
  void,
  { appId: string; feedback: string }
>("review/rejectApp", async ({ appId, feedback }, thunkAPI) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchBucket.fulfilled, (state, { payload }) => {
        state.loading = false;
        state[
          payload.status.toLowerCase() as "pending" | "rejected" | "approved"
        ] = payload.data;
      })
